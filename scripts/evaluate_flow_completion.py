...
def main_worker(args):
    ...
    for index, items in enumerate(test_loader):
        ...
        frames, masks, flows_f, flows_b, video_name = items
        ...
        epe1 = torch.mean(torch.sum((gt_flows_bi[0] - pred_flows_bi[0])**2, dim=2).sqrt()).detach().cpu()
        epe2 = torch.mean(torch.sum((gt_flows_bi[1] - pred_flows_bi[1])**2, dim=2).sqrt()).detach().cpu()

        cur_video_epe.append(epe1.numpy())
        cur_video_epe.append(epe2.numpy())

        total_frame_epe = total_frame_epe+[epe1.numpy()]*gt_flows_bi[0].shape[1]
        total_frame_epe = total_frame_epe+[epe2.numpy()]*gt_flows_bi[0].shape[1]
