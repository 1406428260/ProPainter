...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, args: dict):
        ...
        json_path = os.path.join('./datasets', args['name'], 'train.txt')

        # with open(json_path, 'r') as f:
        #     self.video_train_dict = json.load(f)
        # self.video_names = sorted(list(self.video_train_dict.keys()))
        with open(json_path, 'r') as f:
            self.video_train_dict = f.readlines()
        self.video_names = sorted([x.strip('\n') for x in self.video_train_dict])
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        json_path = os.path.join('./datasets', args['dataset'], 'test.txt')
        with open(json_path, 'r') as f:
            self.video_test_dict = f.readlines()
        self.video_names = sorted([x.strip('\n') for x in self.video_test_dict])
