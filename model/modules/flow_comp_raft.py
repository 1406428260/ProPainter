...
from SEARAFT.raft import SEARAFT
import json
...
def initialize_SEA_RAFT(model_path='weights/Tartan-C-T-TSKH-spring540x960-M.pth', device='cuda'):
    """Initializes the RAFT model.
    """
    cfg_path = 'SEARAFT/spring-M.json'
...
class RAFT_bi(nn.Module):
    ...
    def __init__(self, model_name='sea_raft',  device='cuda'):
        super().__init__()
        if model_name == 'raft':
            model_path = 'weights/raft-things.pth'
            self.fix_raft = initialize_RAFT(model_path, device=device)
        if model_name == 'sea_raft':
            model_path = 'weights/Tartan-C-T-TSKH-spring540x960-M.pth'
            self.fix_raft = initialize_SEA_RAFT(model_path, device=device)
    ...
    def forward(self, gt_local_frames, iters=20):
        ...
        with torch.no_grad():
            ...
            _, gt_flows_forward = self.fix_raft(gtlf_1, gtlf_2, test_mode=True)
            _, gt_flows_backward = self.fix_raft(gtlf_2, gtlf_1, test_mode=True)
