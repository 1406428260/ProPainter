...
def read_frame_from_videos(frame_root, use_frames):
    start_idx, end_idx = use_frames
    if frame_root.endswith(('mp4', 'mov', 'avi', 'MP4', 'MOV', 'AVI')): # input video path
        ...
        frames = []
        frames_name = []
        fr_lst = sorted([x for x in os.listdir(frame_root) if 'png' in x or 'jpg' in x])
        for fr in fr_lst[start_idx:end_idx]:
            frame = cv2.imread(os.path.join(frame_root, fr))
            frame = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            frames.append(frame)
            frames_name.append(fr)
        fps = None
    size = frames[0].size
    return frames, fps, size, video_name, frames_name
...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5, use_frames=[0, -1]):
    start_idx, end_idx = use_frames
    masks_img = []
    masks_dilated = []
    flow_masks = []
    ...
    if mpath.endswith(('jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG')): # input single img path
       masks_img = [Image.open(mpath)]
        ...
        mnames = sorted([x for x in os.listdir(mpath) if 'png' in x or 'jpg' in x])
        for mp in mnames[start_idx:end_idx]:
            masks_img.append(Image.open(os.path.join(mpath, mp)))
          
    for ii, mask_img in enumerate(masks_img):
        if size is not None:
        ...
        # img_org = cv2.imread(f'/data9/zhf/data/xyrs/1/{mnames[start_idx:end_idx][ii]}')
        # img_org = cv2.resize(img_org, mask_img.shape[:2][::-1])
        # res_img = img_org * mask_img[:, :, np.newaxis]
        # cv2.imwrite(f'{mnames[start_idx:end_idx][ii]}', res_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dir_id', type=int, default=1, help='ocr split file info id')
    # parser.add_argument(
    #     '--use_frames', type=list, default=[0, -1], help='Current dir imgs use area')
    parser.add_argument(
    ...
    # set up flow model
    ##############################################
    fix_raft = RAFT_bi(device=device)
    ...
    # if len(args.use_frames) != 2:
    #     args.use_frames = eval(''.join(args.use_frames))

    # dir_id = '1'
    with open(f'tools/{args.dir_id}_info.pkl', 'rb') as f:
    ...
    for use_frames in command_list:
        args.use_frames = use_frames

        frames, fps, size, video_name, frames_name = read_frame_from_videos(args.video, args.use_frames)
        out_size = size
        ...
        frames, size, _ = resize_frames(frames, size)
        ...
        if args.mode == 'video_inpainting':
            ...
            flow_masks, masks_dilated = read_mask(args.mask, frames_len, size,
                                                  ...
                                                  mask_dilates=args.mask_dilation, use_frames=args.use_frames)
            w, h = size
        ...
        with torch.no_grad():
            ...
            if frames.size(1) > short_clip_len:
                ...
                for f in range(0, video_length, short_clip_len):
                    ...
                    if f == 0:
                        flows_f, flows_b = fix_raft(frames[:,f:end_f])
                    else:
                        flows_f, flows_b = fix_raft(frames[:,f-1:end_f])
        ...
        # save each frame
        if args.save_frames:
            assert len(frames_name) == video_length
            for idx in range(video_length):
                f = comp_frames[idx]
                f = cv2.resize(f, out_size, interpolation = cv2.INTER_CUBIC)
                f = cv2.cvtColor(f, cv2.COLOR_BGR2RGB)
                img_save_root = os.path.join(save_root, 'sea_test', frames_name[idx])
                # mask_save_root = os.path.join(save_root, 'masks', frames_name[idx])
                # mask_img = updated_masks[0][idx][0].detach().cpu().numpy()*255
                # imwrite(mask_img, mask_save_root)
